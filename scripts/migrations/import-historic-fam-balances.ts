...
console.log('VITE_SUPABASE_SERVICE_ROLE_KEY exists:', !!process.env.VITE_SUPABASE_SERVICE_ROLE_KEY);  // ✅ Fixed: Check service key, not anon key
console.log('VITE_SUPABASE_ANON_KEY exists:', !!process.env.VITE_SUPABASE_ANON_KEY);  // Added for comparison
...
function parseAmount(amountString: string): number {
  try {
    ...
    if (Math.abs(amount) < 10 && (amountString.includes('Â') || amountString.includes(','))) {
      console.log(`⚠️  Amount parsing: "${amountString}" → "${cleanAmount}" → ${amount}`);
    }
...
async function initializeDatabaseCache(supabase: any, userId: string): Promise<DatabaseCache> {
  ...
  try {
    ...
    if (categories && categories.length > 0) {
      ...
      logger.success(`Found Family Transfer category: ${cache.familyTransferCategoryId}`);
    } else {
      logger.warning('Family Transfer category not found! Make sure it exists.');
    }
...
async function importHistoricFamilyBalances() {
  ...
  try {
    ...
    try {
      ...
      console.log('Sample user IDs in accounts table:', userData?.map(u => u.user_id));
    ...
    if (cache.familyMembers.size === 0) {
      throw new Error('No family members found in database. Make sure you have family members set up.');
    }
    if (!cache.familyTransferCategoryId) {
      throw new Error('Family Transfer category not found in database. Make sure this category exists.');
    }
    ...
    if (processedTransactions.length > 0) {
      ...
      processedTransactions.slice(0, previewCount).forEach((tx, i) => {
        logger.info(`Transaction ${i + 1}:`, {
          ...
          family_member: Object.entries(CONFIG.familyMemberMapping).find(([k, v]) => 
            cache.familyMembers.get(v) === tx.family_member_id
          )?.[0],
      ...
      processedTransactions.forEach(tx => {
        const memberName = Object.entries(CONFIG.familyMemberMapping).find(([k, v]) => 
          cache.familyMembers.get(v) === tx.family_member_id
        )?.[0] || 'Unknown';
        familyMemberCounts[memberName] = (familyMemberCounts[memberName] || 0) + 1;
      });
      ...
      const suspiciousTransactions = processedTransactions.filter(tx => 
        tx.amount > 10000 || tx.amount < 0.01 || isNaN(tx.amount) || isNaN(tx.amount_gbp)
      );
    ...
    if (CONFIG.dryRun) {
      ...
      logger.success(`🎉 Successfully imported ${insertedCount} historic family balance transactions!`);
    }
