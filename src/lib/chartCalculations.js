...
export const chartCalculations = {
  ...
  getLastMonthExpensesByCategory: (transactions, categoryColors = {}) => {
    ...
    const getCategoryDataForMonth = (month, year) => {
      const monthTransactions = getExpenseFilteredTransactions(
        transactions,
      ).filter((t) => {
    ...
    const monthBeforeLastData = getCategoryDataForMonth(
      monthBeforeLast,
      monthBeforeLastYear,
    );
  ...
  getExpensesByTrip: (transactions) => {
    ...
    const tripTransactions = getExpenseFilteredTransactions(
      transactions,
    ).filter((t) => {
  ...
  getMonthlyStats: (transactions) => {
    ...
    currentMonthTransactions
      .filter(
        (t) =>
          t.category?.name !== "Income" && t.category?.name !== "Investment",
      )
      .forEach((t) => {
    ...
    const totalBalance = baseFilteredTransactions.reduce((balance, t) => {
      const amount = t.amount_gbp || 0;
  ...
  get12MonthAverageIncome: (transactions) => {
    ...
    const totalIncome = last12Months.reduce(
