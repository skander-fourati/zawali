...
function mapAccount(account: string, isPersonalCapital: boolean): string {
  ...
  const map = isPersonalCapital
    ? PERSONAL_CAPITAL_ACCOUNT_MAP
    : MONEYHUB_ACCOUNT_MAP;
...
export function parsePersonalCapitalCSV(
  csvContent: string,
): ParsedTransaction[] {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    try {
      ...
      if (columns.length >= 6) {
        ...
        let category = mapPersonalCapitalCategory(
          categoryRaw || "Other / Unknown",
          amount,
        );
        ...
        console.log(
          `Personal Capital: ${date}, ${description}, $${amount} -> £${amountGbp.toFixed(2)}, Category: ${category}`,
        );
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    try {
      ...
      if (columns.length >= 6) {
        ...
        const category = mapMoneyHubCategory(
          categoryRaw || "Other / Unknown",
          amount,
          description,
        );
        ...
        console.log(
          `MoneyHub: ${date}, ${description}, £${amount}, Category: ${category}`,
        );
