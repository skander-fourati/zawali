...
export function ExpensesOverTime({ data, categoryColors = {} }: ExpensesOverTimeProps) {
  ...
  const chartConfig = categoryArray.reduce((config, category, index) => {
    ...
    const finalColor = categoryColors[category] || fallbackColors[index % fallbackColors.length];
